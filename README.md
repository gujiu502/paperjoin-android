...
![按住圖片 1 秒進入多選，再點圖片累加選取](docs/screenshots/multi-select.png)
...
- **拖拉排列與多選**：長按頁面拖拉；按住圖片約 1 秒且不移動即可選取，再點其他圖片累加／取消選取。頁面選單也能移到最前／最後，可選取後批量移除。
...
3. 點縮圖放大檢查，長按拖拉排序；按住圖片約 1 秒不移動可進入多選，接著點其他圖片選取。提前拖動仍照常排序；未選取頁面時，工具列旋轉會套用全部頁面。
...
```bash
./gradlew assembleRelease -PversionName=1.0.2
```

## 驗證

裝置測試涵蓋混合匯入、實際長按拖拉與頁序保存、靜止長按多選、拖動／放手／取消不誤選、旋轉、文字保留、實際圖片壓縮、PNG 透明度、8 種 EXIF 方向、損壞／加密檔案拒絕、中英文方向辨識及中斷寫入的備份恢復。

...
![按住圖片 2.5 秒進入多選，再點圖片累加選取](docs/screenshots/multi-select.png)
...
- **拖拉排列與多選**：長按頁面拖拉；按住圖片約 2.5 秒且不移動即可選取，再點其他圖片累加／取消選取。頁面選單也能移到最前／最後，可選取後批量移除。
...
3. 點縮圖放大檢查，長按拖拉排序；按住圖片約 2.5 秒不移動可進入多選，接著點其他圖片選取。提前拖動仍照常排序；未選取頁面時，工具列旋轉會套用全部頁面。
